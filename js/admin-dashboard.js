...
async function updateAllStudentSemesters() {
  const ok = confirm(
    'Update semester for all students?\n\nThis will increase every student semester by 1. For example, 4 becomes 5 and 8 becomes 9.'
  );
  if (!ok) return;

  setLoading('admin-semester-update-btn', true, 'Update Semester +1 For All Students');
  try {
    ...
    for (const s of students) {
      const currentSemester = parseInt(String(s.semester), 10);
      if (Number.isNaN(currentSemester)) {
        skipped++;
        continue;
      }

      await apiFetch('/admin/students/' + s.id, {
        method: 'PUT',
        body: JSON.stringify({
          name: String(s.name || ''),
          email: String(s.email || ''),
          branch: String(s.branch || ''),
          semester: String(currentSemester + 1),
    ...
    alert(`Semester update complete.\nUpdated: ${updated}\nSkipped: ${skipped}`);
  } catch(e) {
    alert('Semester update failed: ' + e.message);
  } finally {
    setLoading('admin-semester-update-btn', false, 'Update Semester +1 For All Students');
  }
}
